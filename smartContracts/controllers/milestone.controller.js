...
module.exports = {
    deploy: async function (req, res, next) {
        if (!req.body.milestoneOneGoal || !req.body.milestoneTwoGoal ||!req.body.recipientAddress ||!req.body.addFund) {
            res.status(400).send({    
              message: "milestoneOneGoal, milestoneTwoGoal, recipientAddress, addFund can not be empty!"
            });
            return;
          }
        try {
            console.log(req.body.addFund);
            // Get Balance
            ...
            const init = [
              ...
              },
              {
                vname: 'milestone_one',
                type: 'Uint128',
                value: `${req.body.milestoneOneGoal}`,
              },
              {
                vname: 'milestone_two',
                type: 'Uint128',
                value: `${req.body.milestoneTwoGoal}`,
              },
              {
            ...
            const callTx = await deployedContract.call(
              ...
              [],
              {
                ...
                version: VERSION,
                amount: new BN(req.body.addFund),
                gasPrice: myGasPrice,
