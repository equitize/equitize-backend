    ...
    getCampaigns : async (req, res, next) => {
      ...
      checkSCstatus : (req, res, next) => {
        try {
          ...
          if ( SCstatus.CrowdfundingSC.status === true && SCstatus.EquityTokenSC.status === true ) {
                res.status(200).send({
                    startupId: startupId,
                    message: "Equity Token SC and Crowdfunding SC succesfully deployed.",
                    crowdfundingSCaddress: SCstatus.CrowdfundingSC.address,
                    equityTokenSCaddress: SCstatus.EquityTokenSC.address
                });
          } else {
            res.status(500).send("Error deploying Crowdfunding SC and Equity Token SC.")
          }
        } catch (error) {
          next(error)
        }
